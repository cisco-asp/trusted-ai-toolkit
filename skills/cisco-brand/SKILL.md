...
### Base Template
- **Source file**: `Cisco_PowerPoint_Template_DARK_04-01-2026.potx` (dark theme) or `Cisco_PowerPoint_Template_LIGHT_04-01-2026.potx` (light theme)
- **Location**: The templates are bundled with the `pptx` skill at `~/.agents/skills/pptx/templates/`. Always reference the template by its full absolute path:
...
- Do NOT look in the current working directory — the templates are always in the pptx skill's `templates/` directory.
- These are `.potx` (PowerPoint template) files containing slide masters, layouts, theme definitions, and sample slides
